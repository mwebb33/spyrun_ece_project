  ...
  Plugin.prototype = {
    ...
    _drawCountdownLabel: function (secondsElapsed) {
	  var secondsLeft = this.settings.seconds - secondsElapsed,
          label = secondsLeft === 1 ? "Highscore" : "Highscore";
    ...
    _draw: function () {
      var secondsElapsed = Math.round(((new Date().getTime() - this.startedAt.getTime())/1000)),
          endAngle = (Math.PI*3.5) - (((Math.PI*2)/this.settings.seconds) * secondsElapsed); //times10
      this._clearRect();
      this._drawCountdownShape(Math.PI*3.5, false);
	  var random = 3
      if (((secondsElapsed)+random) < this.settings.seconds) {
        this._drawCountdownShape(endAngle, true);
        this._drawCountdownLabel(secondsElapsed);
      } else {
	    var javascriptVariable = ((this.settings.seconds-secondsElapsed));
		//window.location.href = "http://spyrun461l.com/sql.php?name=" + javascriptVariable; 
	  //$.get('sql.php', function(data) {
       //eval(data);
    //});
